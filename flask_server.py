...
def user_loader(username):
    user_record = query_db("SELECT username, role FROM users WHERE username = ?", [username], one=True)
...
def login():
    if request.method == 'POST':
        ...
        user_record = query_db("SELECT username, password, role FROM users WHERE username = ?", [username], one=True)
...
def index():
    ...
    system_uptime = (datetime.now() - datetime.fromtimestamp(query_db("SELECT MIN(timestamp) FROM logs", one=True)[0])).total_seconds()
    return render_template('index.html', role=current_user.role, alerts_count=alerts_count, user_count=user_count, system_uptime=system_uptime)
...
def manage_users():
    ...
    if request.method == 'POST':
        ...
        modify_db("INSERT INTO users (username, password, role) VALUES (?, ?, ?)", [username, password, role])
...
def settings():
    ...
    if request.method == 'POST':
        ...
        modify_db("UPDATE settings SET theme = ?, language = ? WHERE id = 1", [theme, language])
...
if __name__ == '__main__':
    ...
    """)
    cursor.execute("INSERT OR IGNORE INTO settings (id, theme, language) VALUES (1, 'light', 'en')")
    conn.commit()
    conn.close()
