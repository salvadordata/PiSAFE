...
def generateSimplePCMToneData(
    startfreq, endfreq, sampRate, duration, sampWidth, peakLevel, numCh
):
    ...
    for i in range(0, numSamples):
        ...
        for ch in range(numCh):
            sample = int(
                (fade * level * math.sin((freq * 2 * pi * i) / sampRate + phase))
            )
            # print sample
...
def generateDualTonePCMData(
    freq1, freq2, sampRate, duration, sampWidth, peakLevel, numCh
):
        ...
        for ch in range(numCh):
            sample = int(
                (
                    ...
                    * level
                    * (
                        0.5 * math.sin((freq1 * 2 * pi * i) / sampRate + phase)
