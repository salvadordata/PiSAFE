...
def alert_start(JJJHHMM, format="%j%H%M"):
    ...
    utc_dt = datetime.datetime.strptime(JJJHHMM, format).replace(
...
def get_length(TTTT):
    hh, mm = TTTT[:2], TTTT[2:]
    return " ".join(
        [_f for _f in (time_str(int(hh)), time_str(int(mm), type="minute")) if _f]
    )
...
def county_decode(input, COUNTRY):
    ...
    if COUNTRY == "US":
        ...
        return [
            " ".join([_f for _f in (SAME__LOC[P], county) if _f]),
...
def format_message(
    ...
    **kwargs
    ...
    return command.format(
        ...
        LANG=LANG,
        event=get_event(EEE),
        type=get_indicator(EEE),
        end=fn_dt(alert_end(JJJHHMM, TTTT)),
        start=fn_dt(alert_start(JJJHHMM)),
        organization=defs.SAME__ORG[ORG]["NAME"][COUNTRY],
        PSSCCC="-".join(PSSCCC),
        location=get_location(STATION, TYPE),
        date=fn_dt(datetime.datetime.now(), "%c"),
...
def readable_message(
    ...
    MSG = [
        format_message(
            ...
            COUNTRY=COUNTRY,
            LANG=LANG,
            article=defs.MSG__TEXT[LANG][
                defs.SAME__ORG[ORG]["ARTICLE"][COUNTRY]
            ].title(),
            has=(
                defs.MSG__TEXT[LANG]["HAS"]
                if not defs.SAME__ORG[ORG]["PLURAL"]
                else defs.MSG__TEXT[LANG]["HAVE"]
            ),
    ...
    for idx, item in enumerate(PSSCCC):
        ...
        if current_state != state:
            ...
            output = defs.MSG__TEXT[LANG]["MSG2"].format(
                conjunction="" if idx == 0 else defs.MSG__TEXT[LANG]["AND"],
                state=state,
                division=DIVISION,
            )
            MSG += ["".join(output)]
...
def clean_msg(same):
    ...
    # Valid ASCII codes only
    same = "".join([x for x in same if x in valid_chars])
    slen = len(same) - 1
    if same[slen] != "-":
        ridx = same.rfind("-")
        offset = slen - ridx
        if offset <= 8:
            same = "".join(
                [same.ljust(slen + (8 - offset) + 1, "?"), "-"]
            )  # Add final dash and/or pad location field
...
def parse_arguments():
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=" ".join([defs.PROGRAM, defs.VERSION]),
...
def main():
    ...
    if args.msg:
        same_decode(
            args.msg,
            args.lang,
            same_watch=args.same,
            event_watch=args.event,
            text=args.text,
            call=args.call,
